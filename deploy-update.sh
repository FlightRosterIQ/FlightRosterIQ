...
# Deploy updated FlightRosterIQ to DigitalOcean server
# Server IP: 157.245.126.24
...
cd /root
echo "📂 Backing up current FlightRosterIQ..."
if [ -d "FlightRosterIQ" ]; then
    cp -r FlightRosterIQ FlightRosterIQ-backup-$(date +%Y%m%d-%H%M%S)
fi

echo "📦 Extracting new build..."
cd /root/FlightRosterIQ
tar -xzf /root/flightrosteriq-update.tar.gz

echo "🔄 Restarting server..."
pkill -f "node.*crew-server.js" || pkill -f "node.*working-server.js" || true
nohup node crew-server.js > server.log 2>&1 &

echo "✅ Deployment complete!"
echo "🌐 FlightRosterIQ is running at: http://157.245.126.24:8080"
ENDSSH

# Cleanup
rm flightrosteriq-update.tar.gz
